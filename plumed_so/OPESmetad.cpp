...
// PLUMED 2.10+ removed core/Atoms.h; use Action methods instead
#include "tools/Tools.h"  // for kBoltzmann constant
...
//neighbour list stuff
  ...
  // aggregated walker mode to treat upstream aggregator as single CV across walkers
...
//miscellaneous
  ...
  keys.addFlag("AGGREGATED",false,"Set to TRUE if the ARG(s) are aggregated walker variables (e.g. from WALKER_AGGREGATOR); kernels will be deposited as a single shared update across walkers.");
...
//set kbt_
// PLUMED 2.10+ uses getkBT() (lowercase k) on Action class; kBoltzmann is in Tools.h
  kbt_=getkBT();
  // kBoltzmann from Tools.h is in kJ/(mol·K); need to convert to current units
  // Since getkBT() already returns kB*T in correct units, we can derive kB = kBT/T
  // But if T=0, we need the raw constant. Use kBoltzmann which is in PLUMED default units.
  const double kB = kBoltzmann;  // from Tools.h, in kJ/(mol·K)
  double temp=-1;
  parse("TEMP",temp);
  if(temp>0) {
...
//multiple walkers //TODO implement also external mw for cp2k
  ...
  // parse aggregated mode
  aggregated_mode_=false;
  parseFlag("AGGREGATED",aggregated_mode_);
  if(aggregated_mode_) {
    log.printf("  Aggregated walker mode enabled (OPES): kernels will be deposited as a single shared update across walkers\n");
  }
  if(walkers_mpi) {
...
void OPESmetad<mode>::update() {
  // DEBUG: uncomment the following to trace MPI hangs
  // if(comm.Get_rank()==0 && walker_rank_==0) {
  //   std::cerr << "OPES_DEBUG: update() step=" << getStep() << " counter=" << counter_ << std::endl;
  //   std::cerr.flush();
  // }
...
//update variance if adaptive sigma
  ...
  if(adaptive_sigma_) {
    ...
    if(adaptive_counter_<adaptive_sigma_stride_ && counter_==1) { //counter_>1 if restarting
      if(aggregated_mode_ && NumWalkers_>1) {
        // avoid early return in aggregated mode; gate deposit via global vote
        warmup_ready_local = false;
      } else {
...
//do update
  // local deposit predicate
  bool do_deposit_local = ( getStep()%stride_==0 && (excluded_region_==NULL || excluded_region_->get()==0) );
  // Include adaptive warmup readiness in aggregated mode
  if(aggregated_mode_ && NumWalkers_>1) {
    do_deposit_local = do_deposit_local && warmup_ready_local;
  }
  // In aggregated mode with multiple walkers, require a global agreement to deposit
  if(aggregated_mode_ && NumWalkers_>1) {
    int vote_local = do_deposit_local ? 1 : 0;
    int vote_sum = vote_local;
    // DEBUG: uncomment to trace vote
    // if(comm.Get_rank()==0) std::cerr << "OPES_DEBUG: walker=" << walker_rank_ << " vote_local=" << vote_local << " before Sum" << std::endl;
    if(comm.Get_rank()==0) {
      multi_sim_comm.Sum(vote_sum);
    }
    comm.Bcast(vote_sum,0);
    // DEBUG: uncomment to trace vote result
    // if(comm.Get_rank()==0) std::cerr << "OPES_DEBUG: walker=" << walker_rank_ << " vote_sum=" << vote_sum << " after Bcast" << std::endl;
    do_deposit_local = (vote_sum == (int)NumWalkers_);
  }
  if(do_deposit_local) {
    old_KDEnorm_=KDEnorm_;
    ...
    if(NumWalkers_>1) {
      if(comm.Get_rank()==0) {
        if(aggregated_mode_) {
          // In aggregated mode, treat deposition as a single shared kernel.
          // Use representative walker (index 0) height for normalization.
          std::vector<double> tmp_all_height(NumWalkers_,0.0);
          multi_sim_comm.Allgather(height,tmp_all_height);
          sum_heights = tmp_all_height[0];
          sum_heights2 = sum_heights*sum_heights;
        } else {
          // Standard: sum contributions across walkers
    ...
    // Count a single deposition in aggregated mode, otherwise one per walker
    counter_ += (aggregated_mode_ ? 1u : NumWalkers_);
    ...
    if(adaptive_sigma_) {
      ...
      // First-time trigger adjusted for aggregated mode
      if(counter_==1 + (aggregated_mode_ ? 1u : NumWalkers_)) { //first time only
        for(unsigned i=0; i<ncv_; i++) {
  ...
  //add new kernel(s)
    if(NumWalkers_==1) {
      ...
      if(aggregated_mode_) {
        // Deposit a single shared kernel from representative walker 0
        std::vector<double> center_w(all_center.begin()+ncv_*0,all_center.begin()+ncv_*1);
        std::vector<double> sigma_w(all_sigma.begin()+ncv_*0,all_sigma.begin()+ncv_*1);
        addKernel(all_height[0],center_w,sigma_w,all_logweight[0]);
      } else {
        for(unsigned w=0; w<NumWalkers_; w++) {
...
//dump state if requested
    // Only process state writing if a state file is actually configured (wStateStride_ != 0)
    // This avoids unnecessary MPI collectives that can desync with GROMACS checkpointing
    if(wStateStride_ != 0) {
      bool write_state_local = ( (wStateStride_>0 && getStep()%wStateStride_==0) || (wStateStride_==-1 && getCPT()) );
      if(aggregated_mode_ && NumWalkers_>1) {
        // In aggregated mode, synchronize state writing across all walkers.
        // Use Sum to count how many want to write, then only write if unanimous.
        int write_vote = write_state_local ? 1 : 0;
        if(comm.Get_rank()==0) {
          multi_sim_comm.Sum(write_vote);
        }
        comm.Bcast(write_vote,0);
        write_state_local = (write_vote == (int)NumWalkers_);
      }
      if(write_state_local) {
        dumpStateToFile();
      }
    }
}
